...
// int test_ceil(uint val1,float val2);
extern void forkret(void);
static void freeproc(struct proc *p);
uint64 find_page_usage(uint64);
...
found:
  ...
  p->state = USED;
  p->syscall_count = 0;
  p->ticket_val = MAX_TICKETS;
  p->tick = 0;
  p->stride = MAX_TICKETS / p->ticket_val;
  p->pass = p->stride;
...
void exit(int status)
{
  ...
  // If current process is parent, reparent it
  if (p->thread_id == 0)
  {
    reparent(p);
  }
...
void lottery_scheduler(void)
{
  ...
  for (;;)
  {
    ...
    int total_tickets = 0;
    // calculating the total_no_of_tickets in the running process
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
      if (p->state == RUNNABLE)
      {
        total_tickets += p->ticket_val;
      }
      release(&p->lock);
    }

    unsigned short cur_ticket_total = 0;
    unsigned short win_ticket = rand();

    while (win_ticket > (total_tickets + 1))
    {
      win_ticket = rand();
    }
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        cur_ticket_total += p->ticket_val;

        if (cur_ticket_total < win_ticket)
        {
          release(&p->lock);
          continue;
        }
...
void stride_scheduler(void)
{
  ...
  for (;;)
  {
    ...
    {
      acquire(&cur_p->lock);
      cur_p->pass += cur_p->stride;
      cur_p->state = RUNNING;
      c->proc = cur_p;

      cur_p->tick += 1;
      swtch(&c->context, &cur_p->context);
      c->proc = 0;
...
int free_memory_pages_count()
{
  return get_pages();
}

int get_process_count(int param)
{
...
int get_process_info(uint64 n)
{

  struct proc *p = myproc();

  struct pinfo proc_info;
  ...
  proc_info.page_usage = find_page_usage(p->sz);
  return copyout(p->pagetable, n, (char *)&proc_info, sizeof(struct pinfo));
}

uint64 find_page_usage(uint64 v)
{
  return (v / PGSIZE) + ((v % PGSIZE) != 0);
}
...
void print_scheduler_statistics()
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (p->state != UNUSED && p->state != USED)
    {
      printf("%d(%s): tickets: %d, ticks: %d\n", p->pid, p->name, p->ticket_val, p->tick);
    }
    release(&p->lock);
  }
}
...
int sched_tickets(int n)
{
  ...
  pr->ticket_val = n;
  pr->stride = MAX_TICKETS / pr->ticket_val;
  pr->pass += pr->stride;
  return 0;
}
...
int clone(void *stack)
{
  ...
  // mapping current thread's trapframe page to certain user space
